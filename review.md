...
3. **Functional gap — FIXED:** captive portals were never detected, so the sign-in page was never offered (Security finding 6). Detection was delegated to `nmcli networking connectivity check`, which cannot do the job on the target devices; it is now done by the app itself.
4. Several smaller robustness/hardening items below — the error-swallowing cleanup bug and missing initial focus in secondary windows (Accessibility findings 2 and 3) have been fixed.
...
- **Portal redirect targets are vetted before reaching a browser.** The sign-in address comes from a `Location` header written by unauthenticated network equipment, so `safe_portal_url` accepts only absolute `http`/`https` addresses, rejects control characters and over-long values, and resolves relative targets against the probe URL. A `file:`, `data:`, or `javascript:` redirect therefore cannot make the app open attacker-chosen content, and requiring a scheme also guarantees the value can never be read as an option by the browser's own argument parser. When a target is refused the app falls back to `PORTAL_URL`, which a portal intercepts just the same.
- **The browser is launched by argv, detached, and never through a shell.** `launch_portal_browser` uses `asyncio.create_subprocess_exec` with `start_new_session=True`, matching the no-shell rule the `nmcli` path already follows.
...
**6. (Medium) Captive portals were never detected, so the sign-in page was never offered — FIXED**
`get_connectivity` called `nmcli networking connectivity check` and treated its answer as authoritative. Neither half of that works on the target hardware. NetworkManager's connectivity checking is off unless a file under `/etc/NetworkManager/conf.d` sets `connectivity.uri`, which is the default state on Raspberry Pi OS; while it is off, NM reports `full` unconditionally (`ConnectivityCheckEnabled false`, `ConnectivityCheckUri ""`, `Connectivity 4`), so `report_connectivity` took the "Internet access is available" branch on every captive network and the confirm dialog was never shown. Forcing a recheck is separately guarded by PolicyKit (`org.freedesktop.NetworkManager.network-control`) and fails outright with "Error: Not authorized to recheck connectivity", which sent the app down its "Internet access could not be checked" path instead — and where the policy *does* grant it by prompting, it would put an admin password dialog in front of a user who only wanted to get online. Enabling NM's checker or relaxing the PolicyKit rule would both be system-wide changes this app must not make, so detection now happens inside the app: `get_connectivity` issues concurrent plain-HTTP requests to the `PORTAL_PROBES` endpoints, each with a known fixed response, and reads a redirect, a `511`, or any unexpected body as interception. Plain HTTP remains deliberate and is still not a defect — a portal can only intercept unencrypted requests, so an `https://` probe would fail to connect rather than reveal the portal. `PORTAL_URL` is now only the fallback address, used when a portal did not name a sign-in page of its own and quoted to the user when they have to open a browser by hand.

**7. (Low) The sign-in browser is chosen by name rather than by desktop default**
`BROWSER_COMMANDS` tries `firefox` first and only reaches `xdg-open` (the desktop's own configured default) fourth. That is deliberate — Firefox is what ships on the target devices and what their screen-reader users are set up for — but it does mean a user who has deliberately chosen a different default browser gets Firefox anyway for portal sign-in. Python's `webbrowser` module was tried first and rejected: it only registers a GUI browser when `DISPLAY` or `WAYLAND_DISPLAY` is present in the app's own environment, so under a launcher with a trimmed environment it silently degrades to `www-browser` (lynx on Raspberry Pi OS) and blocks in `Popen.wait()` instead of opening anything the user can see.

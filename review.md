...
2. **Security gap:** the CA certificate field for Enterprise/802.1X networks is optional, so the app will happily connect to a network authenticating with an unverified server certificate — this defeats WPA-Enterprise's protection against rogue access points.
...
**1. (Medium) Enterprise networks can connect without CA certificate validation**
`app.py:909-913` — the CA certificate field is marked "recommended," not required (`required=False`), for both PEAP and TTLS. If left blank, `802-1x.ca-cert` is simply omitted from the `nmcli connection modify` call (`app.py:973-974`), and NetworkManager will accept whatever server certificate the AP presents. For WPA-Enterprise, this is the primary protection against a rogue/evil-twin access point capturing credentials — without it, an attacker broadcasting the same SSID can harvest the entered username/password. Consider either requiring a CA cert for PEAP/TTLS, or at minimum showing a stronger warning (similar to the WEP warning at `app.py:1046-1049`) when the user proceeds without one.
